...
type
  TAbDfLZStream = class { LZ77 token stream}
    ...
    public
      ...
      function AddLiteral(aCh : ArithByte) : boolean;
        { returns true if the stream is "full"}
...
begin
  ...
  while (FBitsUsed <> 0) do begin
    if (FBufEnd = FBufPos) then
      obsEmptyBuffer;
    PByte(FBufPos)^ := Byte(FBitBuffer and $FF);
    inc(FBufPos);
    FBitBuffer := FBitBuffer shr 8;
    dec(FBitsUsed, 8);
  end;
...
{$ENDIF}
{--------}
function TAbDfLZStream.AddLiteral(aCh : ArithByte) : boolean;
var
  CurPos : PArithByte;
begin
  {$IFDEF UseLogging}
  ...
  CurPos := FCurPos;
  PByte(CurPos)^ := Byte(true);
  inc(CurPos);
  PArithByte(CurPos)^ := aCh;
  inc(CurPos);
  FCurPos := CurPos;
