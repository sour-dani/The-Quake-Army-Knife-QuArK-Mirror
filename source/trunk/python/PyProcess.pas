...
(*if not ReadFile(Pipe, Buffer^.Data[Buffer^.InQueue], 1, Count, Nil) then
   begin  { broken pipe (end of process) }
    PostMessage(g_Form1Handle, wm_InternalMessage, wppn_EndOfData, LPARAM(@Info));
    Result:=0;
    Exit;
   end;
  if Count=1 then*)
  ...
  if not ReadFile(Pipe, InputMsg, SizeOf(InputMsg), Count, Nil) then
   begin
    ...
    PostMessage(g_Form1.Handle, wm_InternalMessage, wppn_EndOfData, LPARAM(@Info));
    Result:=1;
    Exit;
   end;
  if Count=0 then
     ...
     while (I < Count) do
      begin
       ...
       if not Buffer^.Pending then
        begin
         Buffer^.Pending:=True;
         PostMessage(g_Form1.Handle, wm_InternalMessage, wppn_DataWrite, LPARAM(@Info));
        end;
