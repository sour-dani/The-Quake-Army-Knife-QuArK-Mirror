...
Uses Messages, Windows, SFXmisc, ShellApi, ComObj, ActiveX, ShlObj, Registry, Sysutils;
...
Begin
     ...
     Begin
        ...
        {********************************************************************}
        { Here idBrowse should be destroyed,  Lucjan Lukasik                                          }
        {********************************************************************}
        OleCheck( SHGetMalloc( ppMalloc ) );
...
function checkPyVersion(pyv, pyv_f: byte): boolean;
const
...
begin
  ...
  try
    ...
    if installed then begin
      ...
      begin
        ...
        begin
          if v[i] in digits then
          begin
            pyi:=pyi * 10;
            pyi:=pyi + strtoint(v[i]);
          end;
        end;
        if (pyv_f and PF_ALLOWEQ) > 0 then
        begin
          result:=result or (pyv = pyi);
        end;
        if (pyv_f and PF_ALLOWGT) > 0 then
        begin
          result:=result or (pyv > pyi);
        end;
        if (pyv_f and PF_ALLOWLT) > 0 then
        begin
          result:=result or (pyv < pyi);
        end;
      end;
  ...
  if not result then
  begin
    if MessageBox( 0, STR_NO_PYTHON, STR_W, MB_YESNO or MB_ICONWARNING ) = IDYES then
      result:=true;
  end;
end;
...
begin
   ...
   FGAutoRun := ((flb AND 64) = 64) And (Pos( '!', ExtractFileName( ParamStr( 0 ) ) ) = 1);
   ...
   ReadFile( InFile, pyv, 1, br, nil );     // python version ie 151, 152, 202 ....
   ReadFile( InFile, pyv_f, 1, br, nil );     // python version flags

   if (pyv_f <> 0) then
   begin
     if not checkPyVersion(pyv, pyv_f) then
       Halt;
   end;
